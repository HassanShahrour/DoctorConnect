...
namespace DoctorConnect.Controllers
{
    ...
    public class AvailabilitiesController : Controller
    {
        ...
        public async Task<IActionResult> Availability(string doctorId)
        {
            ...
            {
                Console.WriteLine($"Error in AvailabilitiesController.Availability: {ex}");
                TempData["Error"] = $"An error occurred loading availability: {ex.Message}";
        ...
        public async Task<IActionResult> SaveAvailability(DoctorAvailabilityViewModel model)
        {
            try
            {
                ...
                {
                    TempData["Error"] = "Unable to save. Model validation failed.";
            ...
            {
                Console.WriteLine($"Error in AvailabilitiesController.SaveAvailability: {ex}");
                TempData["Error"] = $"An error occurred saving availability: {ex.Message}";
                return RedirectToAction(nameof(Availability), new { doctorId = model.DoctorId });
            }
        }
    }
